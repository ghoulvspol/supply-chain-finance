#!/usr/bin/env python3
"""
Build a static HTML site from the Supply Chain Finance knowledge graph vault.
...
VAULT = Path("/Users/harden/workspace/supply-chain-finance/vault")
OUT = Path("/Users/harden/workspace/supply-chain-finance/site")
SITE_TITLE = "供应链金融知识图谱"
SITE_LOGO = "S"
...
.type-核心概念{background:#7C5E2A}
.type-参与主体{background:#1A6B7C}
.type-业务模式{background:#2A6B4F}
.type-风控体系{background:#8B2F2F}
.type-政策法规{background:#3A5E8B}
.type-典型案例{background:#4A6E2A}
...
@media(max-width:768px){
  ...
  .stats-row{grid-template-columns:repeat(3,1fr);padding:0 16px;margin:0 0 20px}
  .stat-item{padding:16px 4px}
  .stat-num{font-size:24px;letter-spacing:-1px}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (favBtn && favPop) {
    ...
    favPop.innerHTML =
      '\\u6309 <kbd>' + key + '</kbd> \\u6536\\u85cf\\u672c\\u9875\\u5230\\u6d4f\\u89c8\\u5668\\u4e66\\u7b7e' +
      '<button class="fav-pop-copy" id="fav-pop-copy">\\u590d\\u5236\\u672c\\u9875\\u94fe\\u63a5</button>';
    ...
    if (copyBtn) {
      copyBtn.addEventListener('click', function(e) {
        ...
        navigator.clipboard.writeText(window.location.href).then(function() {
          ...
          setTimeout(function() {
            copyBtn.textContent = '\\u590d\\u5236\\u672c\\u9875\\u94fe\\u63a5';
...
def build_homepage(files, link_map, ref_counts):
    ...
    participant_files = [f for f in files if f["category"] == "participants"]
    model_files = [f for f in files if f["category"] == "models"]
    risk_files = [f for f in files if f["category"] == "risk"]
    policy_files = [f for f in files if f["category"] == "policy"]
    case_files = [f for f in files if f["category"] == "cases"]

    total_cards = len(concept_files) + len(participant_files) + len(model_files) + len(risk_files) + len(policy_files) + len(case_files)
...
<section class="hero-section">
  <div class="hero-eyebrow">Supply Chain Finance Knowledge Graph</div>
  <h1 class="hero-title">供应链金融<span class="gold">知识图谱</span></h1>
  <p class="hero-sub"><b>{len(concept_files)} 个</b>核心概念，<b>{len(model_files)} 种</b>业务模式，<b>{len(case_files)} 个</b>行业案例<br>
  从应收账款融资到数字化平台——构建供应链金融的完整认知体系</p>
</section>
...
<div class="section">
  <div class="section-header">
    <h2 class="section-title">核心概念 TOP</h2>
...
<div class="footer-promo">
  <div class="promo-story">
    <h3>关于本站</h3>
    <p>供应链金融知识图谱是一份结构化的行业知识框架，覆盖核心概念、参与主体、业务模式、风控体系、政策法规和典型案例六大维度。</p>
    <p>从应收账款融资到数字化平台，从传统保理到跨境供应链金融，帮助从业者、研究者和学习者快速建立对供应链金融的全景认知。</p>
    <p class="promo-credit">本站由 <strong>滔哥与Claude Code</strong> 协作构建。</p>
  </div>
</div>
